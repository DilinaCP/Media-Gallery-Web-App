...
export default function ImageDetailPage() {
  ...
  const currentIndex = mockUploads.findIndex((img) => img.id === Number(params.id));

  const [index, setIndex] = useState(currentIndex);

  const image = mockUploads[index];

  const goNext = () => {
    if (index < mockUploads.length - 1) {
      setIndex(index + 1);
    }
  };
  ...
  return (
    <div className="fixed inset-0 bg-transparent bg-opacity-10 flex items-center justify-center z-50 ">
      ...
      <button
        onClick={goPrev}
        disabled={index === 0}
        className="absolute left-6 text-white text-4xl disabled:opacity-30 cursor-pointer"
      >
        ❮
      </button>
      ...
      <button
        onClick={goNext}
        disabled={index === mockUploads.length - 1}
        className="absolute right-6 text-white text-4xl disabled:opacity-30 cursor-pointer"
      >
        ❯
      </button>
    </div>
